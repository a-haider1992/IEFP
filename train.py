...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = TrainImageDataset(
        'cvfr', transforms=transforms.Compose([
    ...
    train_ld = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size)

    # create model
    model = fr_ae_mi_model_1(n_cls=len(train_ld.dataset.classes), args=args)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(grad_scaler, train_ld, model, optimizer_1,
              optimizer_2, epoch, log_file, args)
...
def train(grad_scaler, train_loader, model, optimizer_1, optimizer_2, current_epoch, log_file, args):
    ...
    # pdb.set_trace()
    for i, (xs, ys, real_age_range, _, _) in enumerate(train_loader):
        ...
        log_probs = F.log_softmax(age_logits, dim=1)
        ...
        # mi_est.update(mi_estimation.mean().item(), real_batch_size)
