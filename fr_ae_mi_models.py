...
from fr_metrics import  ArcMarginProduct, AddMarginProduct
...
class fr_ae_mi_model_1(nn.Module):
    ...
    def forward(self, xs, ys=None, emb=False):
        ...
        print(f'Id embedding size {embs_id.shape}')
        print(f'Ae embedding size {embs_ae.shape}')
        mi_upper_bound = self.clubmi.mi_est(embs_ae, embs_id)
        loglikelihood = self.clubmi.loglikeli(embs_ae, embs_id)
        print(f'Likelihood size {loglikelihood.shape}')        
        if emb:
